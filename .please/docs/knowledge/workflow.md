...
```bash
bun run dev                   # Start dev mode (turbo)
turbo run dev --filter=@pleaseai/kb   # Dev for specific package
```
...
turbo run test --filter=@pleaseai/kb  # Test specific package
bun test                      # Run tests directly with bun/vitest
```
